...
<Dropdown bind:show onOpenChange={handleDropdownChange} {align}>
	...
	<div slot="content">
		<DropdownMenu className="{className} font-sans text-xs">
			...
			{#if !($config?.features?.jiaoxiaoai_managed_mode && $user?.role === 'admin') && ($user?.role === 'admin' || $user?.permissions?.workspace?.models || $user?.permissions?.workspace?.knowledge || $user?.permissions?.workspace?.prompts || $user?.permissions?.workspace?.tools || $user?.permissions?.workspace?.skills)}
